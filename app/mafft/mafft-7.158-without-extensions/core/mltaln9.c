...
int addonetip( int njobc, int ***topolc, float **lenc, float **iscorec, int ***topol, float **len, Treedep *dep, int treeout, Addtree *addtree, int iadd, char **name, int *alnleninnode, int *nogaplen )
{
...
//			reflen = alnleninnode[i]; // BUG!!
		...
		if( repnorg == -1 && dep[i].distfromtip * 2 > minscore && nogaplentoadd <= reflen )  // Use this!
//		if( repnorg == -1 && dep[i].distfromtip * 2 > minscore && nogaplen[norg] <= reflen )  // very slow???
...
//		lenc[posinnew][0] = len[i][0] + len[i][1] - minscore / 2; // BUG!! 2014/06/07 ni hakken
		lenc[posinnew][0] = minscore / 2 - dep[nstep-1].distfromtip;
