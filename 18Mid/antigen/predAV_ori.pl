...
#然后，计算序列之间的抗原与遗传关系
########################################################################################################
my ($subtype,$seqFile1,$seqFile2,$antigenicGeneticDiffFile,$libraryDir,$tempDir,$mafftDir)=@ARGV;
if(scalar @ARGV < 7){
	print "Please input the subtype, seqFile1, seqFile2, outputFile,libraryDir,tempDir, mafftDir\n";die;
}
...
align2referSeq($seqFile1,$seqFile1_standard,$referSeqFile,$tempDir,$mafftDir);
align2referSeq($seqFile2,$seqFile2_standard,$referSeqFile,$tempDir,$mafftDir);
...
##################################################################################################
sub align2referSeq{
	my ($seqForAlignFile,$seqAlignFile,$referSeqFile,$tempDir,$mafftDir)=@_;
